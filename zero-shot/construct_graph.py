...
class TaskGraph():
    def __init__(self, dataset='coin', graph_type='overall', graph_modality='video', graph_fusion='weighted-average', alpha=1.0, merge_video_text=True):
        # Init
        ...
        if graph_modality != 'video-text':
            ...
            if merge_video_text:
                ...
                self.alpha = alpha # For weighted average of graph fusion (alpha=video weight)
    ...
    def register_sequence(self, feat_embeds=None, keystep_embeds=None, keystep_sents=None, sim=None, input_modality=None, class_idx=None, make_gt_graph=False, gt_labels=None):
        '''
        ...
        '''
        assert (feat_embeds is not None and keystep_embeds is not None) or (sim is not None)
        ...
        assert self.graph_type == 'overall' or (self.graph_type == 'task' and class_idx is not None)
        if self.graph_type == 'task':
            ...
            if int(class_idx) not in self.sentence_database:
                ...
                assert keystep_sents == self.sentence_database[int(class_idx)] and len(keystep_sents) == len(self.sentence_database[int(class_idx)])
            # Append class_idx and @@@ symbol in the front.
            keystep_sents = [str(class_idx)+'@@@'+x.replace('@@@', ' ') for x in keystep_sents]
        else:
            if self.sentence_database == {}:
                self.sentence_database = keystep_sents
            else:
                assert keystep_sents == self.sentence_database and len(keystep_sents) == len(self.sentence_database)
        ...
        if (feat_embeds is not None and len(feat_embeds) == 1) or (sim is not None and sim.shape[0] == 1):
            return
        ...
        if make_gt_graph:
            ...
            # Remove backgrounds
            best_keysteps = gt_labels[gt_labels != -1]
        # print(best_keysteps.shape)
        # print(best_keysteps)
        dest = None
        for time_idx in range(len(best_keysteps)):
            source = dest
            # if sim[time_idx, best_keysteps[time_idx]] > 0.5:
    ...
    def normalize_graph(self, graph):
        ...
        for node, edges in graph.items():
            ...
            if sum_weights > 0:
                normalized_edges = {neighbor: weight / sum_weights for neighbor, weight in edges.items()}
    ...
    def merge_graphs(self, video_graph, text_graph, alpha):
        ...
        if alpha < 1. and alpha > 0.:
            ...
            nodes = set(video_graph.keys())
        elif alpha == 0.:
            nodes = set(text_graph.keys())
        ...
        for node in nodes:
            ...
            if node in text_graph and alpha < 1.:
                for neighbor, weight in text_graph[node].items():
                    outgoing_edges[neighbor] = outgoing_edges.get(neighbor, 0) + (1 - alpha) * weight

            # Normalize outgoing edge weights
            # print(node, outgoing_edges)
            fused_graph[node] = self.normalize_graph({node: outgoing_edges})[node]

        return fused_graph

    def check_and_finalize(self, apply_log=False):
        if not self.finalized:
            warnings.warn('Task Graph must be finalized for normalization and merging... Doing that now...')
            self.finalize_task_graph(apply_log=apply_log)
    ...
    def find_statistics(self, tail_prob=0):
        ...
        '''
        coin_videoclip_statistics = {0: -23.828975677490234, 5: -3.1052308082580566, 10: -1.9405536651611328, 15: -1.205209732055664, 20: -0.6483414173126221, 25: -0.18889069557189941, 30: 0.21026432514190674, 35: 0.5699708461761475, 40: 0.902841329574585, 45: 1.2174692153930664, 50: 1.5206257104873657, 55: 1.8181183338165283, 60: 2.1148629188537598, 65: 2.4166736602783203, 70: 2.7299509048461914, 75: 3.0637478828430176, 80: 3.432137966156006, 85: 3.8598978519439697, 90: 4.401253700256348, 95: 5.230754852294922, 100: 24.11414337158203}
        crosstask_mil_statistics = {0: -30.90512466430664, 5: -3.146651554107666, 10: -1.8409147024154662, 15: -0.9835262954235077, 20: -0.28699212670326224, 25: 0.3074645400047302, 30: 0.8429063081741333, 35: 1.3513998329639434, 40: 1.8323472976684572, 45: 2.296899402141571, 50: 2.762255907058716, 55: 3.2211565971374516, 60: 3.68434157371521, 65: 4.160336136817933, 70: 4.662180995941162, 75: 5.199650526046753, 80: 5.796158981323243, 85: 6.467047238349915, 90: 7.277130603790284, 95: 8.412010717391968, 100: 16.75782585144043}
        if self.dataset == 'coin':
            assert tail_prob in coin_videoclip_statistics and 100-tail_prob in coin_videoclip_statistics
            ...
            assert tail_prob in crosstask_mil_statistics and 100-tail_prob in crosstask_mil_statistics
    ...
    def finalize_task_graph(self, apply_log=False):
        '''
        Take average
        ...
        if self.graph_modality != 'video-text':
            ...
            self.task_graph = self.merge_graphs(self.task_graph['video'], self.task_graph['text'], self.alpha)
        else:
            self.task_graph['video'] = self.normalize_graph(self.task_graph['video'])
            self.task_graph['text'] = self.normalize_graph(self.task_graph['text'])
            # If not merging then we are ready to use task graph directly
        if apply_log:
            for u in self.task_graph:
                for v in self.task_graph[u]:
                    self.task_graph[u][v] = math.log2(self.task_graph[u][v])
        ...
        if self.graph_type == 'task':
            ...
            for key in self.task_graph:
                class_idx, orig_key = key.split('@@@')
                if int(class_idx) not in per_task_graph:
                    per_task_graph[int(class_idx)] = {}

                for dest_key in self.task_graph[key]:
                    dest_class_idx, dest_orig_key = dest_key.split('@@@')
                    assert dest_class_idx == class_idx

                    if orig_key not in per_task_graph[int(class_idx)]:
                        per_task_graph[int(class_idx)][orig_key] = {}

                    per_task_graph[int(class_idx)][orig_key][dest_orig_key] = self.task_graph[key][dest_key]
    ...
    def visualize_task_graph(self):
        ...
        if self.graph_type == 'task':
            for class_idx in self.task_graph:
                print('For class idx: {}, printing some transitions...'.format(class_idx))
                class_task_graph = self.task_graph[class_idx]
                for node, edges in class_task_graph.items():
                    sorted_edges = sorted(edges, key=lambda x: edges[x], reverse=True)[:3]
        ...
        if self.graph_type == 'task':
            ...
            for class_idx in self.task_graph:
                ...
                for source in self.task_graph[class_idx]:
                    for dest in self.task_graph[class_idx][source]:
                        source_idx = self.sentence_database[class_idx].index(source.split('@@@')[1])
                        dest_idx = self.sentence_database[class_idx].index(dest.split('@@@')[1])
                        assert source_idx != -1 and dest_idx != -1
                        self.adjacency_matrix[class_idx][source_idx][dest_idx] = self.task_graph[class_idx][source][dest]
        else:
            ...
            for source in self.task_graph:
                for dest in self.task_graph[source]:
                    ...
                    assert source_idx != -1 and dest_idx != -1
                    self.adjacency_matrix[source_idx][dest_idx] = self.task_graph[source][dest]
        ...
        while beam and retry_count < num_retries:
            ...
            for probability, path in beam:
                ...
                if current_state == end_state:
                    ...
                    for next_state, transition_prob in transition_matrix[current_state].items():
                        if next_state not in visited:  # Ignore visited states in the current path
    ...
    def beam_search_3(self, start_state, end_state, transition_matrix, beam_width, k):
        """
        Beam search algorithm to find the top-k paths from start state to end state with maximum probability.
    ...
    def beam_search_2(self, start_state, end_state, transition_matrix, beam_width, k):
        """
        Beam search algorithm to find the top-k paths from start state to end state with maximum probability.
        ...
        while beam and retry_count < num_retries:
            ...
            # Remove duplicates from the beam and update the visited set
            new_beam = []
            for p, path in beam:
                if tuple(path) not in visited:
                    new_beam.append((p, path))
    ...
    def predict_keystep(self, feat_embeds=None, keystep_embeds=None, sim=None, keystep_sents=None, input_modality=None, class_idx=None, method='baseline', discard_percentile=40):
        '''
        ...
        if method == 'baseline':
            ...
            for time_idx in range(sim.shape[0]):
                if time_idx == 0:
                    ...
                    belief = sim[time_idx] + 0.20*torch.mv(adj_mat, belief/np.linalg.norm(belief, 1))
            ...
            # exit()
            top_k = self.beam_search('44789@@@Pop the hood of the car', '44789@@@Pull out the dipstick and wipe it down with a dry cloth', curr_task_graph, beam_width=5, k=1)
